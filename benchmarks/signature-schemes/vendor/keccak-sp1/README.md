...
Vendored from `keccak 0.2.2` on crates.io with one change, in `src/backends/soft.rs`: the software
backend's `get_p1600` returns an SP1 `syscall_keccak_permute` wrapper when compiling for the zkVM and
`ROUNDS == 24`.

`sp1-patches` publishes a Keccak patch for `sha3`, which overrides the permutation *inside* `sha3` --
so anything reaching `keccak` directly, as the newer `shake` crate does, never sees it. `ml-dsa` goes
`ml-dsa -> shake -> keccak`, which is why this exists: without it, ML-DSA's `ExpandA` runs SHAKE in
software and the benchmark would understate it by roughly 18x per permutation.

The `ROUNDS == 24` guard matters. `syscall_keccak_permute` *is* the full 24-round Keccak-f[1600], so
reduced-round callers (TurboSHAKE, KangarooTwelve) must keep the software path or they would silently
compute a different function. Note that the upstream `sha3` patch does not make this distinction --
it drops the round count and always does 24, which is correct for SHA3/SHAKE and wrong for anything
reduced-round.

Benchmark-only. Nothing the protocol builds depends on this crate.
