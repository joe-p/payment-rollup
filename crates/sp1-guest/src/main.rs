...
//!
//! Note what is *not* here: no root and no inbox chain is read as the expected answer. The guest
...
pub fn main() {
    ...
    let inbox_anchor: [u8; 32] = sp1_zkvm::io::read_vec()
        .try_into()
        .expect("inbox_anchor must be 32 bytes");
